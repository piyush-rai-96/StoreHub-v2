...
export const DistrictIntelligence: React.FC = () => {
  ...
  interface DIBriefSection { title: string; icon: 'triage' | 'performance' | 'ops' | 'customer' | 'recommendations' | 'traffic' | 'product' | 'external' | 'district' | 'scorecard' | 'drivers'; bullets: string[] }
  interface DIBrief { greeting: string; sections: DIBriefSection[]; closing: string }
  const DISTRICT_BRIEFS: Record<string, DIBrief> = {
    d14: {
      greeting: `District 14 — Tennessee held Excellence Tier at DPI 87 this week, but this brief goes deeper than the headline. Net district revenue of $1.26M beat plan by +8%, driven primarily by Nashville Flagship and Memphis Central. This is the full weekly business intelligence picture: district scorecard, store-by-store breakdown, product performance, traffic analysis, external context, and what drove the results.`,
      sections: [
        {
          title: 'District Weekly Scorecard — vs Plan, LY, Forecast & Cross-District',
          icon: 'scorecard',
          bullets: [
            '<strong>vs Plan:</strong> District net revenue <strong>$1.26M (+8.0% vs plan)</strong>. 6 of 8 stores beat plan. Gross margin <strong>34.2%</strong> (+0.9pp vs plan target of 33.3%). Units sold: +5.8% vs plan. Markdown rate at 11.4% — within the 12% plan cap.',
            '<strong>vs Last Year (LY):</strong> Net revenue <strong>+5.3% vs LY</strong>. Traffic up +2.8% YoY across the district. Conversion improved from 24.1% → <strong>25.4%</strong> (+1.3pp YoY). Average basket size up from $56.40 → $58.90 (+4.4% YoY).',
            '<strong>vs AI Forecast:</strong> +3.1% above the district-level AI demand forecast. The model underestimated the weather and Semi Annual Sale combined effect on Dresses and summer apparel.',
            '<strong>vs Comparable Week (52-wk prior):</strong> <strong>+6.2% net-comp</strong> — strong like-for-like performance across the district.',
            '<strong>Cross-district rank:</strong> District 14 ranked <strong>#2 of 22 districts nationally</strong> on DPI this week, behind only District 19 (Alabama) at DPI 88. District 14 is the top-ranked district in the Southeast region.',
          ],
        },
        {
          title: 'Traffic & Conversion — District View',
          icon: 'traffic',
          bullets: [
            'District total transactions: <strong>14,280 this week</strong> (+2.8% YoY). Peak day: Saturday (+18% vs Monday baseline), driven by Semi Annual Sale and warm weather across Tennessee.',
            'District conversion rate: <strong>25.4%</strong> — above the national chain average of 23.8%. 5 of 8 stores improved conversion vs LY. The two laggards (Johnson City Mall at 17.4%, Clarksville Crossing at 21.6%) are dragging the district average by an estimated 1.2pp.',
            'Fitting room utilization: average 68% across the district during peak hours. Nashville Flagship at 74% (best in district); Johnson City Mall at 41% (worst — fitting room wait time of 18–22 min is deterring try-ons).',
            'UPT district average: <strong>2.6</strong> (+0.2 vs LY). Nashville Flagship and Memphis Central driving the basket. Multi-item attach rates strongest in stores with complete Summer 2 floorsets and active fitting room staffing.',
          ],
        },
        {
          ...
          icon: 'external',
          bullets: [
            '<strong>Weather — Tennessee:</strong> Warm and sunny weekend across the state (Nashville, Memphis, Clarksville 3–4°F above seasonal average). This drove strong performance in Dresses, Swimwear-adjacent, and Summer 2 categories. Estimated weather uplift: +$42K in district revenue vs a baseline-weather scenario. Johnson City had a cooler, cloudier weekend (–2°F vs norm) — a partial explanation for that store\'s demand miss, though execution failures are the primary driver.',
            '<strong>Semi Annual Sale:</strong> Sale activation contributed an estimated <strong>+$118K incremental district revenue</strong> (AI attribution model vs non-sale baseline). High-traffic stores with well-positioned clearance endcaps (Nashville Flagship, Memphis Central, Franklin Town Center) generated 14–16% of their transactions through clearance while maintaining full-price velocity.',
            '<strong>Local Events:</strong> Graduation weekends across Nashville and Memphis metro areas (Vanderbilt, UT, Rhodes College) drove premium-adjacent traffic Saturday AM. Formal-adjacent categories (Blazer, Midi Dress) surged +28% Saturday vs Thursday average at the 4 metro stores.',
            '<strong>No negative external factors</strong> this week — no weather disruptions, no major competitive openings, no supply chain events affecting the district.',
          ],
        },
        {
          title: 'Product Performance — District-Wide Division & Category Breakdown',
          icon: 'product',
          bullets: [
            `<strong>Women's Division (+11.8% vs plan — district leader):</strong> Dresses: +19% vs plan — Summer Midi Dress is the #1 SKU district-wide (combined 284 units, $13,916). Basics: +7.4%. Blazer: +12% in stores with complete floorsets, –8% in stores with partial sets — the floorset completion directly controls this category's performance.`,
            `<strong>Men's Division (+4.2% vs plan):</strong> Denim Slim Fit Dark Wash: +8%. Polo Classic: +6%. Compression Tee: +9% (gym-adjacent weekend traffic across metro stores). Men's Division is consistent across the district — it does not have the store-to-store variance that Women's shows.`,
            '<strong>Kids (+2.8% vs plan):</strong> Color Block Tee and Cargo Shorts leading. Kids Party Dress is the consistent laggard — a district-wide size-run gap (XS, 4T) is limiting conversion. A DC assortment request for these sizes should be submitted at the district level.',
            '<strong>Accessories (–2.4% vs plan — district-wide miss):</strong> Endcap execution is inconsistent across 5 of 8 stores. AI shelf audit shows average Accessories Endcap at 4.2 facings vs the 6-facing POG spec. Facing correction district-wide is estimated to recover <strong>$6,200–$8,400/week</strong> in incremental Accessories revenue.',
            '<strong>Footwear (+8.6% vs plan):</strong> Running Shoes Elite and Canvas Sneakers both strong. Footwear is the most consistently performed category — self-service format is resilient to service and staffing variance.',
          ],
        },
        {
          title: 'Seasonal vs. NOS / Core — District Performance',
          icon: 'drivers',
          bullets: [
            '<strong>Seasonal (Summer 2): 58% of district revenue</strong> — exactly at plan mix. However, this is a district average that masks wide variance: Nashville Flagship at 62% seasonal (above plan), Johnson City Mall at only 29% (well below plan — a direct result of the incomplete floorset).',
            '<strong>NOS / Core: 42% of district revenue.</strong> Core items (V-Neck Basics, Classic Tee, Polo, Slim Fit Denim) are performing strongly across all 8 stores. The NOS base provides a revenue floor even in stores with execution challenges.',
            '<strong>Clearance / Sale:</strong> District-wide markdown rate 11.4% — within the 12% plan cap. Semi Annual Sale clearance velocity is healthy. No over-stacked clearance risk at 7 of 8 stores. Johnson City Mall is the exception — 24% of units are clearance, compressing GM at that location.',
            '<strong>Full-price sell-through on Summer 2 hero items:</strong> District average 88% — ahead of the 85% national chain average. Strong sell-through signals no need for early markdowns on the Summer 2 seasonal assortment.',
          ],
        },
        {
          title: 'Store-by-Store Breakdown — District Performance Matrix',
          icon: 'district',
          bullets: [
            '<strong>Nashville Flagship (SPI 94 — #1):</strong> Net sales $224K, +8.4% vs plan. Conversion 28.4% (district best). VoC 92%. Summer 2 floorset 100%. Fitting room wait: minimal. The district benchmark — 17.8% of district revenue from 1 of 8 stores.',
            '<strong>Memphis Central (SPI 88 — #2):</strong> Net sales $198K, +6.2% vs plan. Conversion 27.1%. VoC 88%. Strong Dresses and Accessories performance. One minor OOS risk in Men\'s Denim (3 washes vs 4 plan) — flag to buyer.',
            '<strong>Franklin Town Center (SPI 82 — #3):</strong> Net sales $171K, +3.8% vs plan. Conversion 24.8%. VoC 86%. Floorset 100%, signage complete. Best-in-district for broadcast acknowledgement compliance (all 4 of 4).',
            '<strong>Murfreesboro Plaza (SPI 78 — #4):</strong> Net sales $158K, +1.4% vs plan. Conversion 23.9%. "Fitting Room Wait" VoC mentions up +19% WoW — early warning signal. Monitor closely; intervene next week if trend continues. This is the same early-stage pattern that preceded the Johnson City Mall crisis.',
            '<strong>Knoxville Centre (SPI 74 — #5):</strong> Net sales $146K, –0.6% vs plan. Marginal miss. Accessories Endcap at 4 facings vs 6 spec — the single biggest correction opportunity at this store. VoC 82%, stable.',
            '<strong>Clarksville Crossing (SPI 68 — #6):</strong> Net sales $134K, –2.8% vs plan. Conversion 21.6%. OOS: 3 Summer 2 key SKUs delayed 48h from DC (OOS adaptation plan awaiting DM approval). VoC "Messy Aisles" up +38% WoW — root cause: cleaning hours cut 4 weeks ago. Restore +2hrs/day this week.',
            '<strong>Chattanooga Square (SPI 62 — #7):</strong> Net sales $121K, –4.2% vs plan. Conversion 19.8%. Floorset at 74% — Women\'s Wall incomplete. VoC 74%. Broadcast acknowledgement: 2 of 4 outstanding. Needs DM check-in this week.',
            '<strong>Johnson City Mall (SPI 58 — #8, Crisis):</strong> Net sales $104K, –12.4% vs plan. Conversion 17.4% (district worst). Floorset 34%. 14 OOS SKUs. Fire exit SEA auto-fail unresolved. VoC 61%. On-site DM intervention today. This store is the district\'s #1 priority.',
          ],
        },
        {
          title: 'What Drove the District — AI Business Analysis',
          icon: 'drivers',
          bullets: [
            '<strong>The district performance is a tale of two clusters:</strong> Stores with complete Summer 2 floorsets (Nashville Flagship, Memphis Central, Franklin Town Center) averaged $197K in revenue and +6.1% vs plan. Stores with incomplete or missing floorsets (Johnson City Mall, Chattanooga Square, Clarksville Crossing) averaged $120K and –6.5% vs plan. The <strong>$77K average revenue gap per store is almost entirely attributable to floorset execution</strong>.',
            '<strong>Semi Annual Sale amplified the execution gap:</strong> Sale-driven traffic at all 8 stores gave each store the same opportunity. The fully-set stores converted it at 27.1% avg; the partially-set stores at 19.6% avg. The same traffic, the same promotional backdrop — radically different conversion because of execution quality.',
            '<strong>Weather was a district-wide tailwind with local variance:</strong> The warm Tennessee weekend benefited all metro stores. Johnson City\'s cooler conditions were a partial headwind, but peer analysis suggests execution failures account for 70%+ of that store\'s underperformance vs the weather-only explanation.',
            '<strong>Accessories facing gap is a district-wide drag:</strong> The –2.4% district miss in Accessories is not store-specific — it is a systemic POG compliance issue across 5 locations. A single district-level broadcast requiring Accessories Endcap facing correction to 6-spec would recover an estimated $6.2–$8.4K/week immediately.',
          ],
        },
        {
          title: 'Cross-District Network Context',
          icon: 'district',
          bullets: [
            '<strong>District 14 national rank: #2 of 22 districts</strong> at DPI 87. District 19 (Alabama) leads at DPI 88. District 14 is the top-ranked district in the Southeast region, ahead of District 8 (Georgia, DPI 84), District 22 (Carolina, DPI 82), and District 11 (Florida, DPI 79).',
            '<strong>Network context — Semi Annual Sale:</strong> District 14 delivered the <strong>highest Semi Annual Sale incremental revenue in the Southeast</strong> this week ($118K attributed to sale activation). The district\'s complete floorset execution was the multiplier — network leadership has flagged District 14 as a best-practice district for the sale rollout.',
            '<strong>SKU-level national insights affecting this district:</strong> The Kids Party Dress size-run gap (XS, 4T) is a national supply chain issue affecting 14 of 22 districts. A district-level escalation will be more effective than store-by-store requests — submit a combined district request to your regional merchant.',
            '<strong>Benchmarking:</strong> District 14 conversion (25.4%) leads the network average (23.8%) by 1.6pp — a meaningful edge. If Johnson City Mall is brought to Stable tier, the district conversion would be ~26.1%, pushing toward the top 5 nationally.',
          ],
        },
        {
          title: 'Critical Triage — Immediate Actions',
          icon: 'triage',
          bullets: [
            '<strong>Johnson City Mall — Fire Exit [BEFORE NOON]:</strong> SEA auto-fail for blocked emergency Exit B. Call SM directly; confirm clearance before noon. Regional VP escalation if unresolved. This is the only zero-tolerance item in the district.',
            '<strong>Clarksville Crossing — OOS Adaptation Plan [5 min action]:</strong> Approve the 3-SKU OOS adaptation plan in your queue. This protects an estimated $2,400 in at-risk revenue for the Semi Annual Sale weekend.',
            '<strong>Clarksville Crossing — Restore cleaning hours:</strong> Add +2hrs/day cleaning at Clarksville Crossing (and confirm Johnson City Mall is doing the same). VoC model: "Messy Aisles" trend reverses within 10 days of restoring hours.',
            '<strong>Murfreesboro Plaza — Early intervention:</strong> Monitor fitting room VoC (+19% WoW). Call the SM today and ask about fitting room staffing. Early-stage intervention prevents this from becoming the next triage story in 4–6 weeks.',
          ],
        },
        {
          title: 'Week Ahead — District Priorities',
          icon: 'recommendations',
          bullets: [
            '<strong>District broadcast (today):</strong> Issue a district-wide broadcast: "Accessories Endcap — 6-Facing Reset Required by EOD Wednesday." Estimated revenue recovery: $6.2–$8.4K/week district-wide.',
            '<strong>Johnson City Mall recovery plan (this week):</strong> DM on-site today. Overnight floorset reset tonight. Fitting room coverage added tomorrow. Deep clean before weekend. Monitor daily — report progress in the district broadcast channel.',
            '<strong>Kids Party Dress size-run — district escalation (Monday):</strong> Submit a combined district-level assortment request to your regional merchant for XS and 4T in Kids Party Dress. Reference the district-wide demand data.',
            '<strong>Chattanooga Square check-in (this week):</strong> The store is trending toward At-Risk. A 30-minute SM call this week and a focus on floorset completion (74% → 100%) will likely stabilize the situation before it requires formal triage.',
            '<strong>Nashville Flagship knowledge-share (schedule this week):</strong> Document their Summer 2 execution playbook. Host a virtual SM session next week using Nashville Flagship as the template. Target audience: Johnson City Mall, Chattanooga Square, Clarksville Crossing.',
          ],
        },
      ],
      closing: `District 14 is a genuinely high-performing district — DPI 87, #2 nationally, and the Southeast leader. The headline metric is earned. Beneath it, the Johnson City Mall crisis and a district-wide Accessories execution gap are the two items to fix this week. Fix Johnson City and the Accessories endcaps, and District 14 is on a credible path to DPI 89+ and national #1 within 3–4 weeks.`,
    },
    d08: {
      greeting: `District 8 — Georgia at DPI 84 (Stable tier). Revenue of $982K (+2.1% WoW) masks margin erosion from shrinkage and staffing gaps. Here is the full district business picture with store-by-store analysis, product performance, and this week's priorities.`,
      sections: [
        {
          title: 'District Weekly Scorecard — vs Plan, LY & Cross-District',
          icon: 'scorecard',
          bullets: [
            '<strong>vs Plan:</strong> District net revenue <strong>$982K (+2.1% vs plan)</strong>. 5 of 7 stores at or above plan. Gross margin <strong>32.8%</strong> — 85bps below plan, driven by shrinkage losses and above-plan markdown rates at the 2 urban flagships.',
            '<strong>vs Last Year (LY):</strong> Revenue +1.4% vs LY. Traffic flat YoY — no growth in foot traffic, which means conversion and basket are driving all the topline improvement. Conversion 23.2% (+0.4pp YoY). Avg basket size up from $54.10 → $56.80 (+5.0% YoY).',
            '<strong>Cross-district rank:</strong> District 8 ranked <strong>#4 of 22 nationally</strong> at DPI 84. Within Southeast region, ranked #2 behind District 14 (Tennessee, DPI 87). The shrinkage and margin gap are the primary barriers to DPI 85+.',
          ],
        },
        {
          title: 'Traffic & Conversion — District View',
          icon: 'traffic',
          bullets: [
            'District total transactions: <strong>10,840 this week</strong> (flat YoY). Flat traffic with improving basket = the right direction, but long-term traffic growth needs a strategy.',
            'Conversion rate: <strong>23.2%</strong> — below the network average of 23.8%. Two stores drive the gap: Augusta Mall (21.4%) and Athens Center (20.8%) — both impacted by checkout wait times. VoC "Checkout Speed" +31% at these two stores.',
            'Urban flagships (Peachtree Plaza, Augusta Mall) drive 62% of district revenue. The concentration risk is significant — a disruption at either store has an outsized district impact.',
          ],
        },
        {
          title: 'Critical Triage — Active Issues',
          icon: 'triage',
          bullets: [
            '<strong>Shrinkage Spike — Peachtree Plaza (+23%) and Savannah Square (+15%):</strong> Combined +18% vs 4-week average. Loss-prevention data indicates afternoon shift (2–5 PM) as the highest-risk window. Estimated monthly impact: <strong>$14K in lost inventory</strong>. Root cause: afternoon understaffing during 2–5 PM — the same gap causing checkout speed complaints.',
            '<strong>VoC: Checkout Speed — Augusta Mall and Athens Center:</strong> Average wait time up from 3.2min → 5.1min during peak hours. Directly correlated with reduced register staffing during 12–2 PM and 5–7 PM. This is the same root cause as the shrinkage — fewer floor associates during afternoon hours.',
            '<strong>Labor Coverage Gap — Macon Point:</strong> 3 weekend shifts uncovered. Weekend foot traffic is 40% higher than weekday — this is a revenue-risk staffing failure. Fill before this weekend using overtime or cross-district coverage.',
          ],
        },
        {
          title: 'Product Performance — District Highlights',
          icon: 'product',
          bullets: [
            `<strong>Women's Division (+3.8% vs plan):</strong> Summer Midi Dress and Basics are the strongest SKUs across the district. Accessories is lagging (–4.2% vs plan) — Endcap facing compliance is 4.1 vs 6 spec at 4 of 7 stores.`,
            `<strong>Men's Division (+1.2% vs plan):</strong> Polo Classic and Compression Tee leading. Men's Denim is flat — similar to the district-wide pattern of limited wash options vs plan.`,
            '<strong>Footwear (+7.4% vs plan):</strong> Consistent outperformer. Running Shoes and Canvas Sneakers both strong across all store tiers.',
            '<strong>Shrinkage impact by category:</strong> Loss-prevention data shows the majority of shrinkage is concentrated in Accessories (38%) and Men\'s basics (22%) — categories with small SKU sizes and easy concealment. Loss-prevention review should prioritize these categories.',
          ],
        },
        {
          title: 'Store-by-Store Snapshot',
          icon: 'district',
          bullets: [
            '<strong>Peachtree Plaza (SPI 86):</strong> Revenue leader at $192K, but GM compressed by shrinkage. Afternoon floor coverage gap is the single highest-priority fix.',
            '<strong>Augusta Mall (SPI 82):</strong> $168K revenue but checkout speed VoC declining. Open a 2nd register during 12–2 PM and 5–7 PM immediately.',
            '<strong>Savannah Square (SPI 78):</strong> $141K, shrinkage +15%. LP audit needed within 48 hours.',
            '<strong>Athens Center (SPI 74):</strong> $128K, checkout VoC declining. Same fix as Augusta: 2nd register during peak.',
            '<strong>Macon Point (SPI 70):</strong> $112K, weekend staffing gap. 3 open shifts — fill before Saturday.',
            '<strong>Columbus Park (SPI 76):</strong> $106K, clean operations, above plan. No active issues.',
            '<strong>Gainesville West (SPI 68):</strong> $94K, below plan (–4.1%). Floorset at 79% — complete the Women\'s Wall by Wednesday.',
          ],
        },
        {
          title: 'Week Ahead — District Priorities',
          icon: 'recommendations',
          bullets: [
            '<strong>Immediate — Afternoon floor coverage:</strong> Increase floor associates at Peachtree Plaza and Augusta Mall during 2–5 PM. This single fix resolves both the shrinkage spike and the checkout speed complaints.',
            '<strong>48 hours — LP audit at Savannah Square:</strong> Deploy loss-prevention audit. Determine if shrinkage is external (shoplifting), internal, or process-related. Adjust approach accordingly.',
            '<strong>Before this weekend — Macon Point staffing:</strong> Fill the 3 open weekend shifts via overtime or cross-district coverage.',
            '<strong>This week — Gainesville West floorset:</strong> Call the SM. Complete Women\'s Wall by Wednesday. Estimated weekly revenue recovery: $3,800–$5,200.',
          ],
        },
      ],
      closing: `District 8 has strong revenue momentum but is bleeding margin through a single root cause: afternoon understaffing at flagship stores. Fix the 2–5 PM floor coverage gap and the district resolves both shrinkage and checkout speed in the same action. DPI 86 is achievable within 3 weeks if the staffing model is corrected.`,
    },
    d22: {
      greeting: `District 22 — Carolina at DPI 82 (Stable tier). Revenue of $1.08M but a planogram compliance drop, a safety incident, and high staff turnover are preventing upward DPI momentum. Here is the full business picture with store-by-store analysis and forward priorities.`,
      sections: [
        {
          title: 'District Weekly Scorecard — vs Plan, LY & Cross-District',
          icon: 'scorecard',
          bullets: [
            '<strong>vs Plan:</strong> District revenue <strong>$1.08M (+1.4% vs plan)</strong>. 4 of 7 stores at or above plan. Gross margin <strong>33.4%</strong> — below plan target, driven by above-plan markdowns at stores with POG compliance gaps.',
            '<strong>vs Last Year (LY):</strong> Revenue flat vs LY (+0.2%). Traffic flat. Conversion stable at 22.8%. Avg basket size slightly up. No growth trajectory — the district is holding ground but not gaining.',
            '<strong>Cross-district rank:</strong> District 22 ranked <strong>#6 of 22 nationally</strong> at DPI 82. Southeast regional rank #3 (behind District 14 and District 8). The 18% quarterly staff turnover rate is the key structural barrier to DPI improvement.',
          ],
        },
        {
          title: 'Critical Triage — Active Issues',
          icon: 'triage',
          bullets: [
            '<strong>Safety Incident — Raleigh Court (wet floor slip):</strong> Investigation required within 24 hours per compliance protocol. Currently unresolved. Assign the investigation team today — missing this deadline creates legal and compliance exposure.',
            '<strong>Planogram Compliance Drop — 5 of 7 stores below target:</strong> Charlotte Hub at 65% compliance, Durham Heights at 68%. Root cause: seasonal Summer 2 reset began without dedicated visual teams at 5 stores. Estimated lost impulse sales: <strong>$8K/week</strong>. Stores with compliant POGs show +12% basket size vs non-compliant.',
            '<strong>Stock Near-Expiry — Wilmington Bay:</strong> 28 items approaching expiry within 48h. Clearance markdown or donation action needed to avoid write-off. Estimated write-off if not actioned: $1,400.',
          ],
        },
        {
          title: 'Product & Traffic Performance',
          icon: 'product',
          bullets: [
            `<strong>Women's Division (+2.8% vs plan):</strong> Performing in stores with compliant POGs. In stores where the Accessories Endcap and Women's Wall reset is complete, Women's is up +6.4% vs plan. In non-compliant stores: –1.2% vs plan. The POG compliance correlation is direct.`,
            'Traffic: <strong>11,200 transactions</strong> (flat YoY). Conversion rate <strong>22.8%</strong> — below network average (23.8%). The checkout experience at Charlotte Hub is the conversion bottleneck — VoC "Checkout Speed" is up +24% WoW at that location.',
            '<strong>Staff turnover impact:</strong> Stores with >30% new hires in the last quarter have 15% lower audit scores and 8% lower conversion rates — the onboarding gap is a measurable performance drag.',
          ],
        },
        {
          title: 'Store-by-Store Snapshot',
          icon: 'district',
          bullets: [
            '<strong>Charlotte Hub (SPI 74):</strong> Revenue leader at $214K but POG compliance at 65% — highest-impact correction in the district. Checkout speed VoC rising.',
            '<strong>Raleigh Court (SPI 78):</strong> $186K, safety incident requires 24h investigation.',
            '<strong>Durham Heights (SPI 68):</strong> $162K, POG compliance 68%. Visual merchandising support needed.',
            '<strong>Greensboro Park (SPI 82):</strong> $148K, clean operations, above plan. Best-practice store in the district.',
            '<strong>Wilmington Bay (SPI 72):</strong> $134K, near-expiry items need action within 48h.',
            '<strong>Raleigh Commons (SPI 76):</strong> $128K, stable. Minor OOS in Women\'s Basics.',
            '<strong>Asheville Center (SPI 70):</strong> $108K, below plan (–5.2%). New SM onboarded 6 weeks ago — coaching support recommended.',
          ],
        },
        {
          title: 'Week Ahead — District Priorities',
          icon: 'recommendations',
          bullets: [
            '<strong>Today — Safety incident (Raleigh Court):</strong> Assign investigation team. Document findings before the 24-hour compliance deadline.',
            '<strong>48 hours — POG reset support (Charlotte Hub, Durham Heights):</strong> Dispatch visual merchandising support. POG compliance directly translates to +12% basket size — this is a high-ROI action.',
            '<strong>48 hours — Wilmington Bay expiry:</strong> Clear 28 near-expiry items via markdown or donation.',
            '<strong>Structural — Staff retention:</strong> Propose a retention incentive at regional review. Pair Asheville Center and Durham Heights with a mentor SM from Greensboro Park (best-in-class execution). High turnover → onboarding gaps → compliance failures is a cycle that only structural retention improvement can break.',
          ],
        },
      ],
      closing: `District 22\'s core challenge is staffing stability — high turnover creates an onboarding gap that suppresses compliance and conversion. The planogram drop is a symptom. Fixing retention is the structural lever; the POG dispatch this week is the bridge fix. DPI 84 is achievable within 4 weeks if both are executed.`,
    },
    d11: {
      greeting: `District 11 — Florida at DPI 79 (Stable tier, lowest in Southeast region). Revenue of $1.52M (highest in region) but execution scores are the worst. This is a "busy but broken" district — strong demand, poor execution translation. Here is the full business picture.`,
      sections: [
        {
          title: 'District Weekly Scorecard — vs Plan, LY & Cross-District',
          icon: 'scorecard',
          bullets: [
            '<strong>vs Plan:</strong> District revenue <strong>$1.52M (+3.8% vs plan)</strong>. However, gross margin <strong>31.2%</strong> — 2.8pp below plan. Spoilage losses are 3× chain average. The top-line is healthy; the margin is a concern.',
            '<strong>vs Last Year (LY):</strong> Revenue +4.2% vs LY. Traffic up +3.1% YoY — the highest traffic growth rate in the Southeast region. But conversion down –0.6pp YoY, and VoC declining across 3 of the 8 stores.',
            '<strong>Cross-district rank:</strong> District 11 ranked <strong>#9 of 22 nationally</strong> at DPI 79. Despite having the highest revenue in the Southeast, the district ranks last in the region on DPI. The disconnect between sales volume and execution quality is the defining challenge.',
          ],
        },
        {
          title: 'Critical Triage — Active Issues',
          icon: 'triage',
          bullets: [
            '<strong>VoC: Product Availability — +26% spike across Miami Central, Orlando Gateway, Tampa Bay Mall:</strong> OOS rate at <strong>6.2%</strong> (highest in the chain). The OOS issue is concentrated in fast-moving Basics and Dress categories. DC delivery frequency needs to increase for the 4 coastal flagship stores.',
            '<strong>Cooler Temp Deviation — Jacksonville Hub (dairy cooler at 48°F vs 41°F threshold):</strong> Maintenance dispatched, estimated 4-hour repair window. At-risk inventory: <strong>$3.2K</strong>. If not resolved within 4 hours, activate spoilage protocol and transfer at-risk inventory to nearest store.',
            '<strong>Promo Endcap Execution Miss — Fort Lauderdale and St. Petersburg:</strong> 4 endcaps incomplete ahead of weekend sale. Revenue impact: estimated $6K if not completed by Friday. Assign dedicated setup crew — these are both high-traffic stores with above-average impulse purchase rates.',
          ],
        },
        {
          title: 'Product & Traffic Performance',
          icon: 'product',
          bullets: [
            '<strong>Coastal flagships (Miami Central, Tampa Bay):</strong> SPI 91 and 87 respectively. Revenue $308K and $264K combined. Strong conversion (27.1% and 26.4%). These two stores are subsidizing the district DPI.',
            '<strong>Inland stores (Jacksonville, Gainesville):</strong> SPI 54 and 61. Combined revenue $198K. Conversion 18.2% and 19.8%. OOS rate 8.4% — double the district average. The inland stores are structurally underserved by the current DM model.',
            `<strong>Women's Division:</strong> +6.4% vs plan at coastal stores; –9.2% at inland stores. The same product, the same promotions — radically different execution outcomes driven by staffing quality and DC service frequency.`,
            'Spoilage losses district-wide: $12,400 this week — <strong>3× the chain average of $4,100</strong>. The Jacksonville cooler issue is an acute event, but the chronic pattern requires a systematic cooler monitoring protocol across all 8 stores.',
          ],
        },
        {
          title: 'Store-by-Store Snapshot',
          icon: 'district',
          bullets: [
            '<strong>Miami Central (SPI 91):</strong> $308K revenue, strong execution. The district anchor.',
            '<strong>Tampa Bay Mall (SPI 87):</strong> $264K revenue, above plan. Endcap compliance the only open item.',
            '<strong>Orlando Gateway (SPI 82):</strong> $218K, OOS rate rising. VoC "Product Availability" up +18% WoW.',
            '<strong>Fort Lauderdale (SPI 76):</strong> $184K, promo endcap incomplete. Must complete before Friday sale.',
            '<strong>St. Petersburg (SPI 72):</strong> $162K, promo endcap incomplete. Same action as Fort Lauderdale.',
            '<strong>Gainesville (SPI 61):</strong> $138K, well below plan. OOS rate 8.1%, conversion 19.8%. Needs structured support.',
            '<strong>Jacksonville Hub (SPI 54):</strong> $112K, cooler temp deviation today. OOS rate 8.8%. District\'s lowest SPI.',
            '<strong>Daytona Center (SPI 68):</strong> $134K, stable. Minor POG compliance gap.',
          ],
        },
        {
          title: 'Week Ahead — District Priorities',
          icon: 'recommendations',
          bullets: [
            '<strong>Immediate — Jacksonville cooler:</strong> Monitor repair. If not resolved in 4 hours, activate spoilage protocol and transfer at-risk inventory.',
            '<strong>Today — Fort Lauderdale and St. Petersburg endcaps:</strong> Assign dedicated setup crew for both stores. Weekend sale revenue at risk if not completed by Friday.',
            '<strong>This week — DC delivery escalation:</strong> Submit a formal request to increase DC delivery frequency for the 4 coastal flagship stores. OOS rate of 6.2% is costing an estimated $18K/week in lost demand.',
            '<strong>Structural (30-day plan):</strong> Propose a district restructure into coastal and inland sub-clusters with dedicated support. The current one-DM model cannot manage the 37-point SPI gap between coastal and inland stores. Present at half-year regional review.',
          ],
        },
      ],
      closing: `District 11 is the highest-revenue, lowest-DPI district in the Southeast — a structural challenge that requires structural solutions. The coastal stores are excellent; the inland stores need dedicated support. This week\'s priorities are the cooler emergency and the promo endcaps. The long-term fix is a district management restructure.`,
    },
    d19: {
      greeting: `District 19 — Alabama at DPI 88 (Excellence tier, #1 in the Southeast region and #1 nationally this week). Revenue of $640K (+3.6% WoW) with best-in-class margin health at 35.1%. Here is the weekly business picture including store-by-store analysis, product performance, and the 3 active items to maintain the position.`,
      sections: [
        {
          title: 'District Weekly Scorecard — vs Plan, LY & Cross-District',
          icon: 'scorecard',
          bullets: [
            '<strong>vs Plan:</strong> District revenue <strong>$640K (+3.6% vs plan)</strong>. All 5 stores at or above plan. Gross margin <strong>35.1%</strong> — best in the Southeast region and top 5% nationally. Markdown rate 9.8% — well below the 12% plan cap.',
            '<strong>vs Last Year (LY):</strong> Revenue +4.8% vs LY. Traffic up +2.1% YoY. Conversion improved from 26.1% → <strong>27.4%</strong> (+1.3pp YoY). Average basket size up +3.8% YoY — consistent across all 5 stores.',
            '<strong>Cross-district rank:</strong> District 19 ranked <strong>#1 of 22 nationally</strong> at DPI 88. Southeast regional leader. 3 consecutive months at DPI 85+. The district is the network template for stable, high-quality execution.',
          ],
        },
        {
          title: 'Active Items — Protect the Position',
          icon: 'triage',
          bullets: [
            '<strong>Audit Backlog — Birmingham Center and Huntsville Plaza (6 overdue audits):</strong> Low-to-medium risk individually but should be cleared by Friday to protect the district\'s 92% audit completion rate — a key contributor to the DPI 88 ranking.',
            '<strong>Receiving Dock Delay — Montgomery Mall (12h behind schedule):</strong> Impacts floor replenishment timing. Monitor closely — escalate to supply chain if not resolved within 24h to prevent OOS on in-demand SKUs before the weekend.',
            '<strong>VoC: Cleanliness — Mobile Bay and Tuscaloosa Walk (+19% negative mentions):</strong> Early-stage signal. This district has a VoC baseline of 85% — the cleanliness signal, if left unaddressed, could erode that advantage within 3–4 weeks. Increase cleaning rounds at peak hours at both stores this week.',
          ],
        },
        {
          title: 'Product & Traffic Performance',
          icon: 'product',
          bullets: [
            `<strong>Women's Division (+5.8% vs plan):</strong> Best-performing division. Summer Midi Dress and Classic Blazer strong across all 5 stores. Floorset completion: 5 of 5 stores at 100% — this is the #1 driver of Women's performance.`,
            `<strong>Men's Division (+4.1% vs plan):</strong> Consistent across all stores. Polo Classic and Denim Slim Fit leading.`,
            '<strong>Accessories (+3.2% vs plan):</strong> Rare district-wide positive result for this category. Accessories Endcap facing compliance at 5.8 facings (vs 6 spec) — closest to spec of any district in the region.',
            'Conversion rate: <strong>27.4%</strong> — highest in the region, 3.6pp above network average. The consistent fitting room staffing practice district-wide (2× coverage during 11am–3pm peak at all 5 stores) is the primary conversion driver.',
          ],
        },
        {
          title: 'Store-by-Store Snapshot',
          icon: 'district',
          bullets: [
            '<strong>Birmingham Center (SPI 89):</strong> Revenue leader at $168K, +6.2% vs plan. 6 overdue audits — only open item. Strong operations otherwise.',
            '<strong>Huntsville Plaza (SPI 88):</strong> $148K, +5.8% vs plan. 6 overdue audits (combined with Birmingham Center). Receiving on schedule.',
            '<strong>Montgomery Mall (SPI 86):</strong> $134K, +2.4% vs plan. Receiving dock 12h delayed — monitor.',
            '<strong>Mobile Bay (SPI 84):</strong> $108K, at plan. VoC Cleanliness signal emerging. Add cleaning hours at peak.',
            '<strong>Tuscaloosa Walk (SPI 82):</strong> $82K, slightly above plan. VoC Cleanliness signal same as Mobile Bay. Same fix.',
          ],
        },
        {
          title: 'Week Ahead — District Priorities',
          icon: 'recommendations',
          bullets: [
            '<strong>This week — Clear audit backlog (Birmingham Center and Huntsville Plaza):</strong> 6 overdue audits. Schedule and complete by Friday to protect 92% audit completion rate.',
            '<strong>24h — Montgomery Mall receiving delay:</strong> Escalate to supply chain if not resolved within 24h. Flag the specific SKUs at OOS risk.',
            '<strong>This week — VoC Cleanliness (Mobile Bay and Tuscaloosa Walk):</strong> Increase cleaning rounds during peak hours. This is an early-stage signal — catch it before it affects the district VoC average.',
            '<strong>Network recognition:</strong> Nominate District 19 execution approach for documentation at regional meeting. Key differentiators: 5-store footprint (manageable), stable staffing (low turnover), strong DM engagement, consistent fitting room staffing. This is a scalable template for underperforming districts like District 11 (Florida) and District 22 (Carolina).',
          ],
        },
      ],
      closing: `District 19 is the national leader this week and has earned it through 3 months of consistent execution. The 3 active items are all manageable and early-stage — address them this week to protect DPI 88. The district\'s biggest contribution this period is being the network playbook — continue documenting the execution practices that make this district reproducibly excellent.`,
    },
  };
            ...
            <div className="bw-modal" onClick={(e) => e.stopPropagation()}>
                ...
                {bwStep === 1 && (
                  <div className="bw-step-content">
                    ...
                    <div className="bw-audience-options">
                      {([
                        { id: 'all-stores', icon: <StoreOutlined sx={{ fontSize: 16 }}/>, title: `All Stores in District (${totalStores})`, desc: 'Reaches every store and their managers' },
                        { id: 'specific-stores', icon: <FilterListOutlined sx={{ fontSize: 16 }}/>, title: 'Specific Stores', desc: 'Pick one or more stores from your district' },
                        { id: 'managers', icon: <GroupOutlined sx={{ fontSize: 16 }}/>, title: 'District Managers Only', desc: 'Send only to selected store / area managers' },
                      ] as const).map(opt => (
                    ...
                    {bwAudience === 'specific-stores' && (
                      <div className="bw-selector">
                        <div className="bw-selector-header">
                          <span className="bw-selector-title">Select stores <span className="bw-selector-count">{bwSelectedStores.length}/{totalStores}</span></span>
                          ...
                          {activeStores.map(s => {
                            ...
                            return (
                              <div key={s.storeNumber} className={`bw-selector-item ${selected ? 'selected' : ''}`} onClick={() =>
                                ...
                                <div className="bw-selector-avatar bw-selector-avatar--store">
                                  <StoreOutlined sx={{ fontSize: 14 }}/>
                                </div>
                                <div className="bw-selector-item-body">
                                  <span className="bw-selector-item-title">{s.storeName}</span>
                                  <div className="bw-selector-item-meta">
                                    <span className="bw-selector-tag">#{s.storeNumber}</span>
                                    <span className={`bw-selector-tag bw-selector-tag--tier ${s.dpiTier === 'Excellence' ? 'elite' : s.dpiTier === 'Stable' ? 'high' : s.dpiTier === 'AtRisk' || s.dpiTier === 'Crisis' ? 'low' : ''}`}>{s.dpiTier}</span>
                                    <span className="bw-selector-tag">DPI {s.dpi}</span>
                                  </div>
                                </div>
                    ...
                    {bwAudience === 'managers' && (
                      <div className="bw-selector">
                        <div className="bw-selector-header">
                          <span className="bw-selector-title">Select managers <span className="bw-selector-count">{bwSelectedManagers.length}/{teamMembers.length}</span></span>
                          <button className="bw-selector-toggle" onClick={() =>
                            ...
                            return (
                              <div key={m.id} className={`bw-selector-item ${selected ? 'selected' : ''}`} onClick={() =>
                                ...
                                <div className="bw-selector-item-body">
                                  ...
                                  <div className="bw-selector-item-meta">
                                    <span className="bw-selector-tag">{m.role}</span>
                                  </div>
                                </div>
                                <div className={`bw-checkbox ${selected ? 'checked' : ''}`}>{selected && <Check sx={{ fontSize: 11 }}/>}</div>
                              </div>
              ...
              <div className="bw-footer">
                ...
                <div className="bw-footer-actions">
                  {bwStep > 1 && (
                    <Button variant="outlined" color="primary" size="large" onClick={() => setBwStep((bwStep - 1) as 1 | 2 | 3)} disabled={bwSending}>
                      Back
                    </Button>
                  )}
                  {bwStep < 3 && (
              ...
              {showTriageDetail === 'oos-risk' && (
                <>
                  ...
                  <div className="investigation-stores">
                    ...
                    <div className="store-list">
                      <div className="store-item">
                        <span className="store-name">WOM-DRS-008 - Summer Midi Dress</span>
                        <span className="store-metric negative">48h delay</span>
                      </div>
                      <div className="store-item">
                        <span className="store-name">WOM-TOP-014 - Women's V-Neck Basics</span>
                        <span className="store-metric negative">48h delay</span>
                      </div>
                      <div className="store-item">
                        <span className="store-name">WOM-BLZ-001 - Women's Classic Blazer</span>
                        <span className="store-metric negative">48h delay</span>
                      </div>
                    </div>
                  </div>
